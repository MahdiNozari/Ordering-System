                                ...
                                <table class="table align-middle">
                                    ...
                                    <tbody>
                                        @foreach ($cart as $key => $item)
                                            <tr>
                                                <th>
                                                    <img class="rounded" src="{{ ImageUrl($item['primary_image']) }}" width="100" alt="" />
                                                </th>
                                                ...
                                                <td>
                                                    <div class="input-counter">
                                                        <a href="{{ route('cart.increment',['product_id' => $key, 'qty' => 1]) }}" class="plus-btn">
                                                            +
                                                        </a>
                                                        <div class="input-number">{{ $item['qty'] }}</div>
                                                        <a href="{{ route('cart.decrement',['product_id' => $key, 'qty' => 1]) }}" class="minus-btn">
                                                            -
                                                        </a>
                                                    </div>
                                                </td>
                            ...
                            <div class="card">
                                <div class="card-body p-4">
                                    ...
                                    <ul class="list-group mt-4">
                                        ...
                                        @php
                                            $coupon_percent = request()->session()->has('coupon')? request()->session()->get('coupon')['percentage'] : 0 ;
                                            $coupon_price = ($cart_total_price * $coupon_percent)/100;
                                        @endphp

                                        @if(request()->session()->has('coupon'))
                                        <li class="list-group-item d-flex justify-content-between">
                                            <div>تخفیف :
                                                <span class="text-danger ms-1">%{{ $coupon_percent }}</span>
                                            </div>
                                            <div class="text-danger">
                                        ...
                                        <li class="list-group-item d-flex justify-content-between">
                                            ...
                                            <div>
                                                {{ number_format($cart_total_price - $coupon_price) }}
                                            </div>
                                        </li>
                                    </ul>
